...
__global__ void kernel_spmv_init_ans(real* d_ans, int M)
{
...
__global__ void kernel_spmv_amb_atomic(real* ans, real* value, unsigned short* col,
                                       const unsigned int* __restrict__ cl,
                                       const int* __restrict__ cs, const real* __restrict__ vector,
                                       unsigned short* d_permutation,
                                       const unsigned short* __restrict__ d_permutation_offset,
                                       int row_num, int seg_size)
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;
...
#pragma unroll
    ...
    do {
        assumed = old;
        old = atomicCAS(address_ull, assumed,
                        __double_as_longlong(answer + __longlong_as_double(assumed)));
    } while (assumed != old);
...
inline void call_kernel_spmv_amb_atomic(real* d_y, sfAMB* mat, real* d_x, sfPlan* plan)
{
    if (mat->block_size == id) {
        kernel_spmv_amb_atomic<id><<<plan->thread_grid, plan->thread_block>>>(
            d_y, mat->d_sellcs_val, mat->d_sellcs_col, mat->d_cl, mat->d_cs, d_x,
            mat->d_s_write_permutation, mat->d_s_write_permutation_offset, mat->c_size * mat->chunk,
            mat->seg_size);
    }
...
template <>
inline void call_kernel_spmv_amb_atomic<MAX_BLOCK_SIZE>(real* d_y, sfAMB* mat, real* d_x,
                                                        sfPlan* plan)
{
...
{
    kernel_spmv_init_ans<<<div_round_up(mat->M, MAX_LOCAL_THREAD_NUM), MAX_LOCAL_THREAD_NUM>>>(
        d_y, mat->M);
