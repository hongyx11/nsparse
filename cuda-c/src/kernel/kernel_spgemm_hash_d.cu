            ...
            switch (i) {
                case 0:
                    ...
                    set_row_nz_bin_pwarp<<<GS, BS, 0, bin->stream[i]>>>(
                        d_arpt, d_acol, d_brpt, d_bcol, bin->d_row_perm, bin->d_row_nz,
                ...
                case 6: {
                    ...
                    if (fail_count > 0) {
                        ...
                        set_row_nz_bin_each_gl<<<GS, BS, 0, bin->stream[i]>>>(
                            d_arpt, d_acol, d_brpt, d_bcol, d_fail_perm, bin->d_row_nz, d_check,
            ...
            switch (i) {
                case 0:
                    ...
                    calculate_value_col_bin_pwarp<<<GS, BS, 0, bin->stream[i]>>>(
                        d_arpt, d_acol, d_aval, d_brpt, d_bcol, d_bval, d_crpt, d_ccol, d_cval,
                        bin->d_row_perm, bin->d_row_nz, bin->bin_offset[i], bin->bin_size[i]);
                    break;
                case 1:
                    BS = 64;
                    GS = bin->bin_size[i];
                ...
                case 6: {
                    ...
                    calculate_value_col_bin_each_gl<<<GS, BS, 0, bin->stream[i]>>>(
                        d_arpt, d_acol, d_aval, d_brpt, d_bcol, d_bval, d_crpt, d_ccol, d_cval,
                        bin->d_row_perm, bin->d_row_nz, d_check, d_value, max_row_nz,
                        bin->bin_offset[i], bin->bin_size[i]);
                    cudaFree(d_check);
                    cudaFree(d_value);
                } break;
