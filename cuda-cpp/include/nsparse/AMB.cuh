...
class AMB
{
   public:
    ...
    void convert_amb_at(CSR<idType, valType> csr_mat, valType* d_x, valType* d_y,
                        Plan<idType>& plan);
...
__global__ void set_segmented_nnz_num(idType* d_rpt, idType* d_col, idType* d_nnz_num,
                                      idType* d_group_seg, idType* d_offset, idType seg_size,
                                      idType seg_num, idType M, idType pad_M, int group_num_col)
{
    idType i = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void set_segmented_col_val(idType* d_rpt, idType* d_col, valType* d_val,
                                      ...
                                      idType* d_group_seg, idType* d_offset, idType M, idType pad_M)
{
    idType i = blockIdx.x;
...
template <class idType, class valType>
void convert_segmented_csr(CSR<idType, valType> csr_mat, idType* d_nnz_num, idType* d_seg_rpt,
                           idType* d_seg_col, valType* d_seg_val, idType seg_size, idType seg_num,
                           idType M, idType pad_M, idType group_num_col)
{
    ...
    BS = NSPARSE_WARP_SIZE;
    GS = div_ceil(M, BS);
    set_segmented_nnz_num<<<GS, BS>>>(csr_mat.d_rpt, csr_mat.d_colids, d_nnz_num, d_group_seg,
                                      d_offset, seg_size, seg_num, M, pad_M, group_num_col);

    /*Set segmented rpt*/
    GS = div_ceil((pad_M * group_num_col + 1), BS);
    ...
    GS = M;
    set_segmented_col_val<<<GS, BS>>>(csr_mat.d_rpt, csr_mat.d_colids, csr_mat.d_values, d_seg_rpt,
...
template <class idType>
__global__ void set_d_check_nnz(idType* d_check_nnz, idType* d_nnz_num, idType pad_M, int SIGMA,
                                idType sigma_block_row)
{
    idType i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= pad_M) {
        return;
...
template <class idType>
void set_check_nnz(idType* d_check_nnz, idType* d_nnz_num, idType sigma_block, idType pad_M,
                   int SIGMA, int group_num_col)
{
    idType GS, BS;
    BS = MAX_LOCAL_THREAD_NUM;
    GS = div_ceil(sigma_block, BS);

    zero_fill<<<GS, BS>>>(d_check_nnz, sigma_block);

    GS = div_ceil(pad_M, BS);
    set_d_check_nnz<<<dim3(GS, group_num_col), dim3(BS, 1)>>>(d_check_nnz, d_nnz_num, pad_M, SIGMA,
                                                              div_ceil(pad_M, SIGMA));
}
...
template <class idType>
void set_sellcs_chunk(idType* d_nnz_num, idType* d_cl, idType* d_cs, idType* elements_num,
...
template <class idType, class valType>
__global__ void set_segmented_sellcs_col_val(idType* d_rpt, idType* d_col, valType* d_val,
                                             idType* d_nnz_num, idType* d_write_permutation,
                                             idType* d_sellcs_col, valType* d_sellcs_val,
                                             idType* d_cl, idType* d_cs, idType group_num_col,
                                             idType pad_M, idType chunk)
{
    idType i = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void set_ushort_col(compIdType* d_us_sellcs_col, idType* d_sellcs_col, idType* d_cs,
                               idType* d_cl, bool* d_is_empty, int group_num_col, idType pad_M,
                               int chunk, idType seg_size)
{
    idType i = blockIdx.x * blockDim.x + threadIdx.x;
...
void set_gcs(idType* d_gcs, bool* d_is_empty, idType chunk_num)
{
    idType GS, BS;
    BS = 256;
    GS = div_ceil(chunk_num, BS);
    init_gcs<<<GS, BS>>>(d_gcs, d_is_empty, chunk_num);
    thrust::inclusive_scan(thrust::device, d_gcs, d_gcs + chunk_num + 1, d_gcs);
}

template <class idType>
__global__ void set_packed_cl_cs(idType* d_packed_cl, idType* d_packed_cs, idType* d_cl,
                                 idType* d_cs, idType* d_gcs, idType chunk_num)
{
    idType i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= chunk_num) {
        return;
...
template <class idType>
__global__ void update_write_permutation(idType* write_permutation, idType* nnz_num,
                                         idType total_pad_row_num, idType pad_M, idType M)
{
    idType i = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void compress_write_permutation(idType* d_write_permutation,
                                           idType* d_full_write_permutation, idType* d_gcs,
                                           idType total_pad_row_num, int chunk)
{
    idType i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= total_pad_row_num) {
...
__global__ void compress_s_write_permutation(compIdType* d_s_write_permutation,
                                             compIdType* d_s_write_permutation_offset,
                                             idType* d_write_permutation, idType c_size, int chunk)
{
    idType i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= c_size * chunk) {
        return;
...
__global__ void set_blocked_cl(compIdType* d_blocked_cl, compIdType* d_blocked_coffset,
                               ...
                               const compIdType* d_s_col_ell, const idType c_size, const int chunk,
...
template <class idType, class compIdType>
void set_blocked_cl_cs(compIdType* d_blocked_cl, compIdType* d_blocked_coffset,
                       idType* d_blocked_cs, const idType* d_packed_cl, const idType* d_packed_cs,
                       const compIdType* d_s_col_ell, const idType c_size, const int chunk,
                       const int block_size, idType* c_nnz)
{
    ...
    BS = 256;
    GS = div_ceil((c_size * chunk), BS);

    set_blocked_cl<<<GS, BS>>>(d_blocked_cl, d_blocked_coffset, d_packed_cl, d_packed_cs,
                               d_s_col_ell, c_size, chunk, block_size);
...
__global__ void set_blocked_col_val(compIdType* d_bs_col_ell, valType* d_b_val_ell,
                                    ...
                                    compIdType* d_s_col_ell, valType* d_val_ell, idType c_size,
                                    int chunk, int block_size)
{
    idType i = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void adjust_blocked_col_val(compIdType* d_bs_col_ell, valType* d_b_val_ell,
                                       compIdType* d_blocked_cl, idType* d_blocked_cs,
                                       compIdType* d_coffset, idType c_size, int chunk,
                                       int block_size, idType N, idType seg_size)
{
    idType i = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    idType it, k, c, h;
    for (k = 0; k < width; k++) {
        c = d_bs_col_ell[d_blocked_cs[chunk_id] / block_size + tid + k * chunk] +
            (d_coffset[chunk_id] * seg_size);
        if (c + block_size > N) {
            d_bs_col_ell[d_blocked_cs[chunk_id] / block_size + tid + k * chunk] =
                (N - (d_coffset[chunk_id] * seg_size) - block_size);
            h = block_size - (N - c);
            for (it = block_size - h - 1; it >= 0; --it) {
                d_b_val_ell[d_blocked_cs[chunk_id] + tid + (k * block_size + h + it) * chunk] =
                    d_b_val_ell[d_blocked_cs[chunk_id] + tid + (k * block_size + it) * chunk];
            }
            for (it = 0; it < h; ++it) {
...
void AMB<idType, compIdType, valType>::evaluate_spmv(valType* d_x, valType* d_y, Plan<idType>& plan)
{
    ...
    for (coe = 2; coe <= MAX_THREAD_BLOCK; coe *= 2) {
        plan_.thread_block = NSPARSE_WARP_SIZE * coe;
        plan_.thread_grid = div_ceil(chunk * c_size, plan_.thread_block);
...
template <class idType, class compIdType, class valType>
void AMB<idType, compIdType, valType>::convert_amb_at(CSR<idType, valType> csr_mat, valType* d_x,
                                                      valType* d_y, Plan<idType>& plan)
{
    idType GS, BS;
    idType i;
    ...
    if (SIGMA > 1) {
        ...
        for (i = 0; i < group_num_col; i++) {
            ...
            while (start < M) {
                ...
                if (check_nnz[i * div_ceil(pad_M, SIGMA) + start / SIGMA] > 0) {
    ...
    set_segmented_sellcs_col_val<<<GS, BS>>>(
        d_seg_rpt, d_seg_col, d_seg_val, d_nnz_num, d_full_write_permutation, d_nb_sellcs_col,
        d_nb_sellcs_val, d_full_cl, d_full_cs, group_num_col, pad_M, chunk);
    ...
    if (plan.isPlan == false) {
        for (block_size = 1; block_size <= max_block; block_size++) {
            ...
            adjust_blocked_col_val<<<div_ceil((c_size * chunk), BS), BS>>>(
                d_sellcs_col, d_sellcs_val, d_cl, d_cs, d_coffset, c_size, chunk, block_size, N,
                seg_size);
            cudaDeviceSynchronize();
            ...
            footprint +=
                (c_size)*chunk * sizeof(compIdType) + (c_size) * sizeof(compIdType);  // permutation
    ...
    set_blocked_col_val<<<div_ceil((c_size * chunk), BS), BS>>>(
        ...
        d_nb_sellcs_val, c_size, chunk, (block_size));
    adjust_blocked_col_val<<<div_ceil((c_size * chunk), BS), BS>>>(
        d_sellcs_col, d_sellcs_val, d_cl, d_cs, d_coffset, c_size, chunk, block_size, N, seg_size);
...
template <class idType, class compIdType, class valType>
void AMB<idType, compIdType, valType>::convert_from_csr(CSR<idType, valType> mat,
                                                        Plan<idType>& plan, valType* d_x)
{
    int i;
...
template <int block_size, class idType, class compIdType, class valType, class AddOperation,
          class MultOperation, class AAddOperation>
__global__ void kernel_spmv_amb_atomic(
    valType* ans, const valType* value, const compIdType* col, const compIdType* __restrict__ cl,
    const compIdType* __restrict__ coffset, const idType* __restrict__ cs,
    const valType* __restrict__ vector, const compIdType* d_permutation,
    const compIdType* __restrict__ d_permutation_offset, idType row_num, idType seg_size,
    AddOperation addop, MultOperation mulop, AAddOperation aaddop)
{
    ...
    idType start = cs[c_index] + (threadIdx.x & (NSPARSE_WARP_SIZE - 1));
    idType colstart = (cs[c_index] / block_size) + (threadIdx.x & (NSPARSE_WARP_SIZE - 1));
...
#pragma unroll
    for (int b = 0; b < block_size; ++b) {
        ...
        start += NSPARSE_WARP_SIZE;
    }
    colstart += NSPARSE_WARP_SIZE;
...
{
   public:
    inline static void f(const valType* d_x, valType* d_y, const Plan<idType>& plan,
                         AMB<idType, compIdType, valType>* mat)
    {
        if (bs == mat->block_size) {
            kernel_spmv_amb_atomic<bs><<<plan.thread_grid, plan.thread_block>>>(
                d_y, mat->d_sellcs_val, mat->d_sellcs_col, mat->d_cl, mat->d_coffset, mat->d_cs,
                d_x, mat->d_s_write_permutation, mat->d_s_write_permutation_offset,
                mat->c_size * mat->chunk, mat->seg_size, Add<valType>(), Multiply<valType>(),
                AtomicAdd<valType>());
        }
   ...
   public:
                         ...
                         AMB<idType, compIdType, valType>* mat)
    {
        if (MAX_BLOCK_SIZE == mat->block_size) {
            kernel_spmv_amb_atomic<MAX_BLOCK_SIZE><<<plan.thread_grid, plan.thread_block>>>(
                d_y, mat->d_sellcs_val, mat->d_sellcs_col, mat->d_cl, mat->d_coffset, mat->d_cs,
                d_x, mat->d_s_write_permutation, mat->d_s_write_permutation_offset,
                mat->c_size * mat->chunk, mat->seg_size, Add<valType>(), Multiply<valType>(),
                AtomicAdd<valType>());
...
template <class idType, class compIdType, class valType>
void AMB<idType, compIdType, valType>::spmv(const valType* d_x, valType* d_y,
                                            const Plan<idType>& plan)
{
    kernel_spmv_init_ans<<<div_ceil(M, MAX_LOCAL_THREAD_NUM), MAX_LOCAL_THREAD_NUM>>>(d_y, M);
