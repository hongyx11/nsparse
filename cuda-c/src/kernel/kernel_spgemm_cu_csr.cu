...
__global__ void set_intprod_per_row(int* d_arpt, int* d_acol, const int* __restrict__ d_brpt,
                                    long long int* d_max_row_nz, int M)
{
...
void get_spgemm_flop(sfCSR* a, sfCSR* b, int M, long long int* flop)
{
    int GS, BS;
    ...
    set_intprod_per_row<<<GS, BS>>>(a->d_rpt, a->d_col, b->d_rpt, d_max_row_nz, M);
...
void spgemm_kernel_cu_csr(sfCSR* a, sfCSR* b, sfCSR* c, cusparseHandle_t* cusparseHandle,
                          cusparseOperation_t* trans_a, cusparseOperation_t* trans_b,
                          cusparseMatDescr_t* descr_a, cusparseMatDescr_t* descr_b)
{
    int m, n, k;
    int base_c, nnz_c;
    ...
    // msec[it++] = (float)(end.tv_sec - start.tv_sec) * 1000 +
    ...
    /* Count nnz of C */
    // gettimeofday(&start, NULL);
    status = cusparseXcsrgemmNnz(*cusparseHandle, *trans_a, *trans_b, m, n, k, *descr_a, a->nnz,
                                 a->d_rpt, a->d_col, *descr_b, b->nnz, b->d_rpt, b->d_col, descr_c,
                                 c->d_rpt, nnzTotalDevHostPtr);
    ...
    // gettimeofday(&start, NULL);
#ifdef FLOAT
    status = cusparseScsrgemm(*cusparseHandle, *trans_a, *trans_b, m, n, k, *descr_a, a->nnz,
                              a->d_val, a->d_rpt, a->d_col, *descr_b, b->nnz, b->d_val, b->d_rpt,
                              b->d_col, descr_c, c->d_val, c->d_rpt, c->d_col);
#else
    status = cusparseDcsrgemm(*cusparseHandle, *trans_a, *trans_b, m, n, k, *descr_a, a->nnz,
                              a->d_val, a->d_rpt, a->d_col, *descr_b, b->nnz, b->d_val, b->d_rpt,
                              b->d_col, descr_c, c->d_val, c->d_rpt, c->d_col);
#endif
...
void spgemm_cu_csr(sfCSR* a, sfCSR* b, sfCSR* c)
{
    ...
    spgemm_kernel_cu_csr(a, b, c, &cusparseHandle, &trans_a, &trans_b, &descr_a, &descr_b);
    cudaThreadSynchronize();
