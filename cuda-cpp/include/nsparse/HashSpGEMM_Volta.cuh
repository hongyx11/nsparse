...
constexpr int TS_N_P = 16;   // Table Size for Numeric in PWARP per row
                                 ...
                                 idType* d_row_nz, idType bin_offset, idType M)
{
    ...
    for (j = d_arpt[rid] + wid; j < d_arpt[rid + 1]; j += wnum) {
        acol = ld_gbl_col(d_acolids + j);
        for (k = d_brpt[acol] + tid; k < d_brpt[acol + 1]; k += NSPARSE_WARP_SIZE) {
            bcol = d_bcolids[k];
            key = bcol;
    ...
    for (j = NSPARSE_WARP_SIZE / 2; j >= 1; j /= 2) {
        nz += __shfl_xor_sync(0xffffffff, nz, j);
    }
                                ...
                                idType* d_nz, idType bin_offset, idType bin_size)
{
    ...
    if (threadIdx.x < NSPARSE_WARP_SIZE) {
        for (j = tid; j < SH_ROW; j += NSPARSE_WARP_SIZE) {
            if (id_table[j] != -1) {
                index = atomicAdd(d_nz + rid, 1);
