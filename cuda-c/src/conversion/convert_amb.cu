...
__global__ void set_sellcs_col_val(int* d_rpt, int* d_col, real* d_val, int* d_nnz_num,
                                   int* d_write_permutation, int* d_sellcs_col, real* d_sellcs_val,
                                   int* d_cl, int* d_cs, int group_num_col, int pad_M, int M,
                                   int chunk)
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void set_segmented_nnz_num(int* d_rpt, int* d_col, int* d_nnz_num, int* d_group_seg,
                                      int* d_offset, size_t seg_size, size_t seg_num, int M,
                                      int pad_M, int group_num_col)
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void set_segmented_col_val(int* d_rpt, int* d_col, real* d_val, int* d_seg_rpt,
                                      int* d_seg_col, real* d_seg_val, int* d_group_seg,
                                      int* d_offset, int M, int pad_M)
{
    int i = blockIdx.x;
...
void convert_segmented_csr(sfCSR* csr_mat, int* d_nnz_num, int* d_seg_rpt, int* d_seg_col,
                           real* d_seg_val, size_t seg_size, size_t seg_num, int M, int pad_M,
                           int group_num_col)
{
    size_t GS, BS;
    ...
    set_segmented_nnz_num<<<GS, BS>>>(csr_mat->d_rpt, csr_mat->d_col, d_nnz_num, d_group_seg,
                                      d_offset, seg_size, seg_num, M, pad_M, group_num_col);
    ...
    thrust::inclusive_scan(thrust::device, d_seg_rpt, d_seg_rpt + pad_M * group_num_col + 1,
                           d_seg_rpt);

    /*Set segmented col and val*/
    GS = M;
    set_segmented_col_val<<<GS, BS>>>(csr_mat->d_rpt, csr_mat->d_col, csr_mat->d_val, d_seg_rpt,
                                      d_seg_col, d_seg_val, d_group_seg, d_offset, M, pad_M);
...
__global__ void compress_write_permutation(int* d_write_permutation, int* d_full_write_permutation,
                                           int* d_gcs, int total_pad_row_num, int chunk)
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= total_pad_row_num) {
        return;
...
__global__ void compress_s_write_permutation(unsigned short* d_s_write_permutation,
                                             ...
                                             int* d_write_permutation, int c_size, int chunk)
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= c_size * chunk) {
        return;
    ...
    if (i % chunk == 0) {
        d_s_write_permutation_offset[chunk_id] =
            (unsigned short)(d_write_permutation[i] / USHORT_MAX);
    }
}
...
__global__ void set_ushort_col(unsigned short* d_us_sellcs_col, int* d_sellcs_col, int* d_cs,
                               int* d_cl, BOOL* d_is_empty, int group_num_col, int pad_M, int chunk,
                               int seg_size)
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void set_packed_cl_cs(int* d_packed_cl, int* d_packed_cs, int* d_cl, int* d_cs,
                                 int* d_gcs, int chunk_num)
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= chunk_num) {
        return;
...
__global__ void set_blocked_cl(unsigned int* d_blocked_cl, int* d_packed_cl, int* d_packed_cs,
                               unsigned short* d_s_col_ell, int c_size, int chunk, int block_size)
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= c_size * chunk) {
        return;
    ...
    if (j == 0) {
        max_width = width;
        d_blocked_cl[i / chunk] =
            ((max_width / block_size) - 1) | ((d_packed_cl[i / chunk] >> SCL_BORDER) << SCL_BORDER);
    }
}

__global__ void init_blocked_cs(int* d_cs, unsigned int* d_cl, int c_size, int chunk,
                                int block_size)
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= c_size) {
...
void set_blocked_cl_cs(unsigned int* d_blocked_cl, int* d_blocked_cs, int* d_packed_cl,
                       int* d_packed_cs, unsigned short* d_s_col_ell, int c_size, int chunk,
                       int block_size, int* c_nnz)
{
    ...
    set_blocked_cl<<<GS, BS>>>(d_blocked_cl, d_packed_cl, d_packed_cs, d_s_col_ell, c_size, chunk,
                               block_size);
    ...
    int r_size;
    checkCudaErrors(
        cudaMemcpy(c_nnz, d_blocked_cs + (c_size - 1), sizeof(int), cudaMemcpyDeviceToHost));
    checkCudaErrors(cudaMemcpy(&r_size, d_blocked_cl + (c_size - 1), sizeof(unsigned int),
...
__global__ void set_blocked_col_val(unsigned short* d_bs_col_ell, real* d_b_val_ell,
                                    unsigned int* d_blocked_cl, int* d_blocked_cs, int* d_packed_cl,
                                    int* d_packed_cs, unsigned short* d_s_col_ell, real* d_val_ell,
                                    int c_size, int chunk, int block_size)
{
    ...
    for (k = 0; k < cl_width / block_size; k++) {
        if (it < ((d_packed_cl[chunk_id] & SCL_BIT) + 1)) {
            ...
            d_b_val_ell[d_blocked_cs[chunk_id] + tid + (k * block_size + (c - base)) * chunk] =
                d_val_ell[d_packed_cs[chunk_id] + tid + it * chunk];
            it++;
            for (h = c - base + 1; h < block_size; h++) {
                if (it < ((d_packed_cl[chunk_id] & SCL_BIT) + 1)) {
                    if (d_s_col_ell[d_packed_cs[chunk_id] + tid + it * chunk] - base == h) {
                        d_b_val_ell[d_blocked_cs[chunk_id] + tid + (k * block_size + h) * chunk] =
                            d_val_ell[d_packed_cs[chunk_id] + tid + it * chunk];
                        it++;
                    } else {
                        d_b_val_ell[d_blocked_cs[chunk_id] + tid + (k * block_size + h) * chunk] =
            ...
            d_bs_col_ell[d_blocked_cs[chunk_id] / block_size + tid + k * chunk] =
                (d_s_col_ell[d_packed_cs[chunk_id] + tid +
                             ((d_packed_cl[chunk_id] & SCL_BIT)) * chunk] /
                 block_size) *
                block_size;
...
__global__ void set_d_check_nnz(int* d_check_nnz, int* d_nnz_num, int pad_M, int SIGMA,
                                int sigma_block_row)
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= pad_M) {
...
void set_check_nnz(int* d_check_nnz, int* d_nnz_num, int sigma_block, int pad_M, int SIGMA,
                   int group_num_col)
{
    int GS, BS;
    BS = MAX_LOCAL_THREAD_NUM;
    ...
    set_d_check_nnz<<<dim3(GS, group_num_col), dim3(BS, 1)>>>(d_check_nnz, d_nnz_num, pad_M, SIGMA,
                                                              div_round_up(pad_M, SIGMA));
}
...
void convert_amb_at(sfCSR* csr_mat, sfAMB* mat, real* d_x, real* d_y, float* min_msec, sfPlan* plan)
{
    int GS, BS;
    int i;
    ...
    convert_segmented_csr(csr_mat, d_nnz_num, d_seg_rpt, d_seg_col, d_seg_val, mat->seg_size,
                          mat->seg_num, M, pad_M, mat->group_num_col);
            ...
            while (start < M) {
                ...
                if (check_nnz[i * div_round_up(pad_M, SIGMA) + start / SIGMA] > 0) {
                    thrust::stable_sort_by_key(
                        dev_nnz_num + i * pad_M + start, dev_nnz_num + i * pad_M + end,
                        dev_full_write_permutation + i * pad_M + start, thrust::greater<int>());
                }
                start += SIGMA;
            }
        }
    ...
    GS = div_round_up(total_pad_row_num, BS);
    set_sellcs_col_val<<<GS, BS>>>(d_seg_rpt, d_seg_col, d_seg_val, d_nnz_num,
                                   d_full_write_permutation, d_col_ell, d_nb_sellcs_val, d_full_cl,
                                   d_full_cs, mat->group_num_col, pad_M, M, chunk);
    ...
    get_c_size<<<div_round_up((total_pad_row_num / chunk), BS), BS>>>(d_c_size, d_full_cl,
                                                                      total_pad_row_num / chunk);
    ...
    set_ushort_col<<<div_round_up(total_pad_row_num, BS), BS>>>(
        d_nbs_sellcs_col, d_col_ell, d_full_cs, d_full_cl, d_is_empty, mat->group_num_col, pad_M,
        chunk, mat->seg_size);
    ...
    set_packed_cl_cs<<<div_round_up((total_pad_row_num / chunk), BS), BS>>>(
        d_packed_cl, d_packed_cs, d_full_cl, d_full_cs, d_gcs, total_pad_row_num / chunk);
    ...
    update_write_permutation<<<div_round_up(total_pad_row_num, BS), BS>>>(
        d_full_write_permutation, d_nnz_num, total_pad_row_num, pad_M);
    checkCudaErrors(
        cudaMalloc((void**)&(mat->d_write_permutation), sizeof(int) * (mat->c_size * chunk)));
    compress_write_permutation<<<div_round_up(total_pad_row_num, BS), BS>>>(
        mat->d_write_permutation, d_full_write_permutation, d_gcs, total_pad_row_num, chunk);

    checkCudaErrors(cudaMalloc((void**)&(mat->d_s_write_permutation),
                               sizeof(unsigned short) * (mat->c_size * chunk)));
    checkCudaErrors(cudaMalloc((void**)&(mat->d_s_write_permutation_offset),
                               sizeof(unsigned short) * mat->c_size));
    compress_s_write_permutation<<<div_round_up((mat->c_size * chunk), BS), BS>>>(
        mat->d_s_write_permutation, mat->d_s_write_permutation_offset, mat->d_write_permutation,
        mat->c_size, mat->chunk);
    ...
    if (plan->isPlan == FALSE) {
        for (block_size = 1; block_size <= MAX_BLOCK_SIZE; block_size++) {
            ...
            c_nnz = 0;
            set_blocked_cl_cs(mat->d_cl, mat->d_cs, d_packed_cl, d_packed_cs, d_nbs_sellcs_col,
                              mat->c_size, chunk, block_size, &c_nnz);

            mat->nnz = c_nnz;
            checkCudaErrors(cudaMalloc((void**)&(mat->d_sellcs_col),
                                       sizeof(unsigned short) * c_nnz / block_size));
            checkCudaErrors(cudaMalloc((void**)&(mat->d_sellcs_val), sizeof(real) * c_nnz));

            set_blocked_col_val<<<div_round_up((mat->c_size * chunk), BS), BS>>>(
                mat->d_sellcs_col, mat->d_sellcs_val, mat->d_cl, mat->d_cs, d_packed_cl,
                d_packed_cs, d_nbs_sellcs_col, d_nb_sellcs_val, mat->c_size, chunk, block_size);
            cudaThreadSynchronize();
            ...
            footprint += (mat->c_size) * mat->chunk * sizeof(unsigned short) +
                         (mat->c_size) * sizeof(unsigned short);         // permutation
            footprint += (mat->c_size) * mat->chunk * sizeof(real) * 2;  // output
            footprint += (mat->M) * sizeof(real) * 2;                    // input + output_init
            // printf("%s, %d, %d, %d\n", mat->matrix_name, mat->seg_size,
            // mat->block_size, footprint);
    ...
    c_nnz = 0;
    set_blocked_cl_cs(mat->d_cl, mat->d_cs, d_packed_cl, d_packed_cs, d_nbs_sellcs_col, mat->c_size,
                      chunk, mat->block_size, &c_nnz);
    ...
    mat->nnz = c_nnz;
    checkCudaErrors(cudaMalloc((void**)&(mat->d_sellcs_col),
                               sizeof(unsigned short) * c_nnz / (mat->block_size)));
    checkCudaErrors(cudaMalloc((void**)&(mat->d_sellcs_val), sizeof(real) * c_nnz));

    set_blocked_col_val<<<div_round_up((mat->c_size * chunk), BS), BS>>>(
        mat->d_sellcs_col, mat->d_sellcs_val, mat->d_cl, mat->d_cs, d_packed_cl, d_packed_cs,
        d_nbs_sellcs_col, d_nb_sellcs_val, mat->c_size, chunk, (mat->block_size));
